...
export function Wormhole({ active, progress }: Props) {
  ...
  useFrame(({ clock }) => {
    ...
    groupRef.current.children.forEach((child) => {
      ...
      if (mesh.userData.isRing) {
        ...
        if (z > 5) z -= count * 1.18
        mesh.position.z = z
        mesh.rotation.z = t * (0.28 + mesh.userData.idx * 0.004) + mesh.userData.idx * 0.13
        const s = 1 + Math.sin(t * 1.8 + mesh.userData.idx * 0.31) * 0.05 + progress * 0.42
  ...
  return (
    <group ref={groupRef} position={[0, 1.65, 0]}>
      ...
      <mesh position={[0, 0, -count * 0.57]}>
        ...
        <meshStandardMaterial color="#a8d8ff" emissive="#3a7bff" emissiveIntensity={1.4 + progress * 2.5} transparent opacity={0.38} depthWrite={false} />
      </mesh>

      {rings.map((r, i) => (
        <mesh key={i} position={[0, 0, r.z]} rotation-z={i * 0.13} userData={{ isRing: true, idx: i }}>
          <torusGeometry args={[r.radius, 0.07 + (i % 4) * 0.018 + progress * 0.04, 10, 64]} />
          <meshStandardMaterial
            color={new THREE.Color(`hsl(${r.hue}, ${r.sat}%, ${r.light}%)`)}
            emissive={new THREE.Color(`hsl(${r.hue}, 100%, 58%)`)}
