 ...
 * rooftop hazard beacon; size and beacon presence come from a seed, so the
 * same seed always looks the same.
 * @param props - Livery color and deterministic seed
 * @returns Skyscraper group
 */
export function ProceduralSkyscraper({ color = '#3a4a6a', seed = 42 }: { color?: string; seed?: number }) {
...
export function SkyscraperRenderer({ entity }: EntityRendererProps) {
  ...
  return <ModelLoader src={modelRegistry['cityIntro/skyscraper'].path} fallback={<ProceduralSkyscraper color={color} seed={seed} />} />
}
